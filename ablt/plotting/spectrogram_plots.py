...
def spectrogram(title, directories, spectrogram, fs, hop_length, F0_estimate=None, show=True, save=False, plot_title=''):
    ...
    save_function(save, directories['plot']['spectrogram'],
                  title, plot_title=plot_title, default_title='Spectrogram')
...
def note_spectrogram(title, directories, spectrogram, fs, hop_length, notes, unk_notes, show=True, save=False, plot_title=''):
    ...
    save_function(save, directories['plot']['note_spectrogram'], title,
                  plot_title=plot_title, default_title='Spectrogram_Notes')
...
def note_comparison_spectrogram(title, directories, spectrogram, fs, hop_length, F0_estimate, notes, unk_notes, show=True, save=False, plot_title=''):
    ...
    form_beat_grid_spectrogram(
        title, directories, ax[0], spectrogram, fs, hop_length)
    ...
    form_beat_grid_spectrogram(
        title, directories, ax[1], spectrogram, fs, hop_length)

    form_pitch_track(F0_estimate, ax[1], label='pYIN estimation')

    ax[1].legend(loc=1, fontsize=15)

    save_function(save, directories['plot']['spectral_comparison'],
                  title, plot_title=plot_title, default_title='Note_Comparison')
