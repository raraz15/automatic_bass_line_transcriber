...
def adaptive_voiced_region_quantization(pitch_track, track_scale, quarter_beat_positions, filter_unk=False, length_threshold=8, epsilon=4):
    """
    Aplies adaptive quantization to voiced regions accoring to the algorithm 1.
        ...
        -----------
            ...
            filter_unk (boolean, default=False): determines whether to filter out unknown frequencies
    ...
    pitch_track_quantized = uniform_quantization(pitch_track, track_scale[1], good_regions, epsilon)
    ...
    pitch_track_quantized = uniform_quantization(pitch_track_quantized, track_scale[1], okay_regions, epsilon)

    # Silence bad regions
    pitch_track_quantized = region_silencer(pitch_track_quantized, bad_regions)

    if filter_unk: # Filter unk frequencies
        pitch_track_quantized = unk_filter(pitch_track_quantized, track_scale)
